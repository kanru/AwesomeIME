...
public class KeyboardSwitcher {
    ...
    KeyboardSwitcher(AwesomeIME context) {
        mContext = context;
    ...
    boolean onKey(int key) {
        ...
        switch (mSymbolsModeState) {
            case SYMBOLS_MODE_STATE_BEGIN:
                if (key != AwesomeIME.KEYCODE_SPACE && key != AwesomeIME.KEYCODE_ENTER && key > 0) {
                    mSymbolsModeState = SYMBOLS_MODE_STATE_SYMBOL;
                }
                break;
            case SYMBOLS_MODE_STATE_SYMBOL:
                if (key == AwesomeIME.KEYCODE_ENTER || key == AwesomeIME.KEYCODE_SPACE) return true;
                break;
        }
        return false;
    }
